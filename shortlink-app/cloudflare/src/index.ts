// ============================================================
// Cloudflare Worker — ShortLink API Bridge
// Endpoints:
//   GET  /redirect/:slug          → redirect + catat klik
...
// ── Auth guard: baca header Authorization: Bearer <token> ──
// Token = session ID yang disimpan di KV
...
async function log(
  ...
  await env.DB.prepare(`
    INSERT INTO activity_logs
      (actor_id, actor_email, actor_role, action, target_type, target_id, target_label, detail)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
    actor.id, actor.email, actor.role,
    action, target_type, target_id, target_label, detail ?? null
  ).run();
}
  ...
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (method === 'GET' && path.startsWith('/r/')) {
      ...
      if (!original) {
        ...
        ).bind(slug).first<{ id: string; original_url: string }>();
      ...
      // Catat klik di background
      if (linkId) {
        const ip = req.headers.get('CF-Connecting-IP');
        const ua = req.headers.get('User-Agent');
        const ref = req.headers.get('Referer');
        env.DB.prepare(
          'INSERT INTO clicks (link_id, ip, user_agent, referer) VALUES (?,?,?,?)'
        ).bind(linkId, ip, ua, ref).run();
        env.DB.prepare(
    ...
    if (method === 'POST' && path === '/api/auth/request-token') {
      ...
      // Store token info for Next.js to send email
      return json({ token, email, userId: user!.id });
    }

    // ── AUTH: Verify token → buat session ─────────────────────
    if (method === 'POST' && path === '/api/auth/verify-token') {
      ...
      await env.CACHE.put(
        ...
        JSON.stringify(user),
        { expirationTtl: 7 * 24 * 3600 } // 7 hari
      );
    ...
    if (method === 'POST' && path === '/api/links') {
      ...
      const result = await env.DB.prepare(
        'INSERT INTO links (slug, original_url, user_id) VALUES (?,?,?) RETURNING *'
      ).bind(slug, original_url, actor.id).first<{
        id: string; slug: string; original_url: string; created_at: string;
      }>();
    ...
    if (method === 'GET' && path === '/api/links') {
      ...
      if (actor.role === 'user') {
        rows = await env.DB.prepare(`
          SELECT l.*, u.email as user_email FROM links l
          JOIN users u ON u.id = l.user_id
          WHERE l.user_id = ?
          ORDER BY l.created_at DESC LIMIT ? OFFSET ?
        `).bind(actor.id, limit, offset).all();
      } else {
        rows = await env.DB.prepare(`
          SELECT l.*, u.email as user_email, u.role as user_role FROM links l
          JOIN users u ON u.id = l.user_id
          ORDER BY l.created_at DESC LIMIT ? OFFSET ?
    ...
    if (method === 'DELETE' && path.startsWith('/api/links/')) {
      ...
      ).bind(linkId).first<{ id: string; slug: string; original_url: string; user_id: string }>();
    ...
    if (method === 'GET' && path.match(/^\/api\/links\/[^/]+\/stats$/)) {
      ...
      const link = await env.DB.prepare(
        'SELECT * FROM links WHERE id = ?'
      ).bind(linkId).first<{ user_id: string; click_count: number; slug: string }>();

      if (!link) return err('Link not found', 404);
      if (actor.role === 'user' && link.user_id !== actor.id) {
    ...
    if (method === 'PATCH' && path.match(/^\/api\/users\/[^/]+\/role$/)) {
      ...
      ).bind(targetId).first<User>();
      if (!target) return err('User not found', 404);

      const oldRole = target.role;
      await env.DB.prepare(
        'UPDATE users SET role = ?, updated_at = ? WHERE id = ?'
      ).bind(role, new Date().toISOString(), targetId).run();

      // Update session cache jika ada
      // (session akan expired alami, next login sudah role baru)

      const action = role > oldRole ? 'role_upgrade' : 'role_downgrade';
      await log(
        env, actor, action, 'user',
    ...
    if (method === 'GET' && path === '/api/logs') {
      ...
      let query = `
        ...
        ${search ? "AND (actor_email LIKE '%" + search.replace(/'/g, '') + "%' OR target_label LIKE '%" + search.replace(/'/g, '') + "%')" : ''}
