...
export default function AdminLogsPage() {
  ...
  useEffect(() => {
    getMe()
      .then(u => {
        if (u.role !== 'super_admin') { window.location.href = '/dashboard'; }
      })
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        <div className="bg-white rounded-2xl border border-purple-200 overflow-hidden">
          ...
          <div className="px-6 py-4 border-b border-gray-100 bg-purple-50
            ...
            <div>
              <h1 className="font-semibold text-purple-900">Log aktivitas sistem</h1>
              <p className="text-xs text-purple-500 mt-0.5">{total.toLocaleString('id')} total entri</p>
            </div>
            <div className="flex gap-2">
              <select
                value={action}
                onChange={e => { setAction(e.target.value); setPage(1); }}
                className="text-sm px-3 py-2 rounded-lg border border-purple-200 bg-white
              ...
              <input
                type="text"
                placeholder="Cari email / slug..."
                value={search}
                onChange={e => { setSearch(e.target.value); setPage(1); }}
                className="text-sm px-3 py-2 rounded-lg border border-purple-200
                           focus:outline-none focus:ring-2 focus:ring-purple-400 w-44"
              />
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="py-12 text-center text-gray-400 text-sm">Tidak ada log ditemukan.</div>
          ) : (
            <div className="divide-y divide-gray-100">
              {logs.map(log => {
                const meta = ACTION_META[log.action] ?? {
                  label: log.action, color: 'bg-gray-100 text-gray-500', icon: '•'
                };
                return (
                  <div key={log.id}
                    ...
                    <span className={`text-xs px-2.5 py-1 rounded-md font-medium
                                      whitespace-nowrap mt-0.5 ${meta.color}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-gray-700">
                        ...
                        {' '}
                        {log.action === 'create_link' && (
                          <>membuat <code className="bg-gray-100 px-1.5 py-0.5 rounded text-xs">
                            /r/{log.target_label}
                          </code></>
                        )}
                        {log.action === 'delete_link' && (
                          <>menghapus <code className="bg-gray-100 px-1.5 py-0.5 rounded text-xs">
                            /r/{log.target_label}
                          </code></>
                        )}
                        {(log.action === 'role_upgrade' || log.action === 'role_downgrade') && (
                          <>mengubah role <span className="font-medium">{log.target_label}</span>
                          {' '}({log.detail})</>
                        )}
                      </p>
                      {log.detail && log.action.includes('link') && (
