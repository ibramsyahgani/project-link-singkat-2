...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    getMe()
      .then(u => {
        if (u.role === 'user') { window.location.href = '/dashboard'; return; }
        setMe(u);
        return getUsers();
      })
      .then(u => u && setUsers(u))
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <div>
              <h1 className="font-semibold text-gray-900">Kelola user</h1>
              <p className="text-xs text-gray-400 mt-0.5">{users.length} total user</p>
            </div>
            <input
              type="text"
              ...
              return (
                <div key={user.id}
                  ...
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center
                                   ...
                                   ${ROLE_BADGE[user.role] || 'bg-gray-100 text-gray-500'}`}>
                    {initials}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate">
                      {user.email}
                      {isSelf && <span className="ml-2 text-xs text-gray-400">(kamu)</span>}
                    </p>
